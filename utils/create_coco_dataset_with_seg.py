...
def main(
    ...
    detection_processor = AutoProcessor.from_pretrained(detection_model_id, local_files_only=True)
    detection_model = GroundingDinoForObjectDetection.from_pretrained(detection_model_id, local_files_only=True).to('cuda')

    segmentation_processor = AutoProcessor.from_pretrained(segmentation_model_id)
    segmentation_model = AutoModelForMaskGeneration.from_pretrained(segmentation_model_id).to('cuda')
    ...
    for sample in tqdm(dataset):
        ...
        inputs = detection_processor(images=image, text=text_prompt, return_tensors='pt').to('cuda')
        ...
        for box, score, label in zip(results['boxes'], results['scores'], results['labels']):
            ...
            seg_inputs = segmentation_processor(images=image, input_boxes=[[box.tolist()]], return_tensors='pt').to('cuda')
            seg_outputs = segmentation_model(**seg_inputs)
            masks = segmentation_processor.post_process_masks(
                masks=seg_outputs.pred_masks,
                original_sizes=seg_inputs.original_sizes,
                reshaped_input_sizes=seg_inputs.reshaped_input_sizes,
            )[0]
            masks = masks.squeeze().cpu().float()
            masks = masks.mean(dim=0)
            masks = (masks > 0).int()
            masks = masks.numpy().astype(np.uint8)
            masks = masks[int(box[1]):int(box[3]) + 1, int(box[0]):int(box[2]) + 1]
